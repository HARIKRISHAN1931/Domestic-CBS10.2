...
test.describe('State Master > Negative @regression', () => {
  test('should reject state master', async ({ checkerAuthenticatedPage }) => {
    const rows   = await ExcelHelper.readSheet<StateMasterData>(DATA_FILE, 'Negative');
    const data   = rows[0];
