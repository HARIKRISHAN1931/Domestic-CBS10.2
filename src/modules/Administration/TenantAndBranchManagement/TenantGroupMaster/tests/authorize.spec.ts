...
test.describe('Tenant Group Master > Authorize @regression', () => {
  test('should authorize tenant group master', async ({ authenticatedPage }) => {
    const rows   = await ExcelHelper.readSheet<TenantGroupMasterData>(DATA_FILE, 'Authorize');
    const data   = rows[0];
    const screen = new TenantGroupMasterPage(authenticatedPage);
    await test.step('Navigate', () => NAV(authenticatedPage));
    const toast = await test.step('Approve', () => screen.approve(data.institutionId!));
