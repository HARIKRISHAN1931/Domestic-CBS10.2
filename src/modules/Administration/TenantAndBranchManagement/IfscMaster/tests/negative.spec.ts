...
test.describe('IFSC Master > Negative @regression', () => {
  test('should reject ifsc master', async ({ authenticatedPage }) => {
    const rows   = await ExcelHelper.readSheet<IfscMasterData>(DATA_FILE, 'Negative');
    const data   = rows[0];
    const screen = new IfscMasterPage(authenticatedPage);
    await test.step('Navigate', () => NAV(authenticatedPage));
    const toast = await test.step('Reject', () => screen.reject(data.ifscCd!, 'Rejected by automation'));
